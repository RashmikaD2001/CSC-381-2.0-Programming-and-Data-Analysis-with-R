...
### 1.2 Corona virus pandemic in Lithuania
...
lt_confirm_plotly
```
This plot represent confirmed corona virus cases between 2020 January 22 to 2023 March 09. 

      Confirmed Cases = No. of People Currently Infected + No. of People Cured + No. of People Died From Coronavirus
      
There are increasing and decreasing patterns of confirmed cases during the time period. We can observe a rapid increasing of a confirmed cases in first four months of the year 2022. This may result because of remove the restrictions impose for Corona virus by the government.
...
Above figure display active, recovery and death cases occurred due to the Corona virus. There are several increases and decreases patterns in three plots.

    Active Cases = No. of People Currently Infected

We can observe maximum increase of death during the end of year 2020 and starting of the year 2021 which is above 40 everyday. Maximum number of deaths recorded in a single is 62 cases on 2020 December 26. But the active cases are below 3500 this time period. Which is not a good thing. During these vaccination is program had just begun. 

We can observe maximum increase during the begining of 2022 in active cases plot.Peek occurred on 2022 February 03 with 15412. Even we observe a such increase in number confirmed cases number of deaths are always lesser than 40. At the begining of 2022 Lithuanian government has gave 2 doses of vaccination to many citizens. 

We can observe the increasing pattern of active, recovery and death cases during the winter of 2020 and 2021.

Starting from the month August in year 2021. There is no record about recovery cases. All the data are zero.
...
```
Lithuanian government announced a national lockdown on 4 November 2020. The lockdown was set to last until 29 November, but was later lengthened until 1 July 2021. During this time period we can observe relatively flat line in the plot. As well as during the first four months of year 2022 we can exponential spreading of Corona virus. When we come to the July 2022 there is slow growth in active cases. During this time most of the citizens have taken the vaccine. 
...
For the analysis considered neighbouring countries are **Belarus**, **Latvia**, **Estonia** and  **Poland**. Among these countries Latvia and Moldova are have population approximately equal to Lithuania. But Belarus and Poland are not. Poland have more than **nine** times higher population than Lithuania. 
...
boltic_confirm <- boltic_countries %>% ggplot(aes(x = date, y = confirmed, color = country)) +
  geom_line() + 
  geom_hline(yintercept = 1144.188, linetype = "dashed") +
  labs(x = "Date", y = "Confirmed Cases in Baltic Region") +
  theme_minimal() +
  theme(legend.position = "none")
  
boltic_death <- boltic_countries %>% ggplot(aes(x = date, y = death, color = country)) +
    geom_line() + geom_hline(yintercept = 8.418198, linetype = "dashed") +
  labs(x = "Date", y = "Death in Baltic Region") +
  theme_minimal()
  

boltic_confirm_plotly <- ggplotly(boltic_confirm)
boltic_death_plotly <- ggplotly(boltic_death)
  
 subplot(
  boltic_confirm_plotly,
  boltic_death_plotly,
  nrows = 2,  
  shareX = TRUE,
  titleX = TRUE,
  ...
  layout(
    title = "Plots Confirm and Death cases in Baltic Region",  
    title_x = 0.5, 
    title_y = 0.95
  )
